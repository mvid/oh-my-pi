...
export class SelectorController {
	...
	showUsageDashboard(reports: UsageReport[]): void {
		...
		const { displayReports, usageModelSelectors } = resolveUsageView(reports, {
			...
			showUsageModels: this.ctx.settings.get("display.showUsageModels"),
			getUsageReportingModelSelectors: next => this.ctx.session.getUsageReportingModelSelectors(next),
		});
		const done = () => {
			overlayHandle?.hide();
			this.focusActiveEditorArea();
