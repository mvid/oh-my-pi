	...
	async prompt(text: string, options?: PromptOptions): Promise<boolean> {
		...
		if (this.isStreaming) {
			...
			if (streamingBehavior === "followUp") {
				await this.#queueUserMessage(expandedText, options?.images, "followUp", submittedAt, userInitiated);
			} else {
				await this.#queueUserMessage(expandedText, options?.images, "steer", submittedAt, userInitiated);
			}
			return true;
		}
		...
		if (this.isStreaming) {
			...
			await this.#queueUserMessage(expandedText, options?.images, streamingBehavior, submittedAt, userInitiated, {
