...
import type { PanelSettings } from "../panel";
import {
...
export const SETTINGS_SCHEMA = {
	...
	"advisor.lateConcern": {
		...
		ui: {
			...
			options: [
				{
					value: "preserve",
					label: "Preserve as card",
					description: "Default. Show the concern; act on it on the next turn.",
				},
				{ value: "steer", label: "Steer a turn", description: "Wake the agent to address the concern now." },
			],
			condition: "advisorEnabled",
		},
	},
	...
	panel: { type: "record", default: EMPTY_PANEL_SETTINGS },
	...
	"tier.autoFastMode": {
		...
		ui: {
			tab: "model",
			group: "Sampling",
			label: "Auto Fast Mode While Active",
			description:
				"Use priority processing for primary-session requests for the configured duration after a user prompt. It never applies to tasks, advisors, panels, or side-channel turns.",
		},
	},
	"tier.autoFastModeDurationMinutes": {
		type: "number",
		default: 20,
		ui: {
			tab: "model",
			group: "Sampling",
			label: "Auto Fast Mode Duration",
			description: "Minutes to keep automatic priority processing active after a user prompt",
			options: [
