...
Python prelude helpers include `agent(prompt, *, agent="task", model=None, label=None, schema=None, schema_mode=None, isolated=None, apply=None, merge=None, timeout=None, handle=False)`. It synchronously calls the host bridge and returns final text, or parsed data when `schema` is supplied. `model` overrides the agent selector for this call; `timeout` caps subagent wall-clock seconds, with exactly zero disabling and every positive value bounded to at least 1 ms. `schema_mode` selects permissive or strict structured-output handling; the isolation/apply/merge flags control task worktree behavior. With `handle=True`, it returns a DAG node dict (`{"text", "output", "handle", "id", "agent", "model", "family"}`) whose handle is the recoverable `agent://<id>` URI; `model` and `family` identify the served route, and parsed output is stored under `"data"` when available.
