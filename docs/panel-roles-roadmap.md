...
An independent role has at least two members and no `persona` fields. Resolve each model first, then require distinct catalog taxonomy class values from `classifyModel(model.provider, model.id, { lenient: true })`. That follows canonical lineage across providers and proxy names: direct Kimi and Fireworks Kimi are both `kimi`; Claude, GPT, and Kimi are distinct. An unknown family fails closed rather than pretending two opaque names provide diversity.
...
An optional `minFamilies` sets a floor on distinct resolved families that fails resolution when a lineup collapses below it, which is how a `personas` role asks for diversity it does not get by strategy. A floor larger than the member count is rejected at parse time. Where a lineup claims diversity — one family per seat under `independent`, or any floor — an unknown family fails closed; a single-seat lineup claims none, so an unclassified model resolves.
...
2. Resolve the selected panel role and every member selector through the existing model registry before dispatch. Reject unavailable models, invalid effort levels, malformed role shapes, and independent lineups with duplicate or unknown catalog taxonomy classes before any participant starts. Prepare an immutable plan before TUI review, then dispatch that exact plan after approval rather than resolving it again.
...
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1    | Define panel-role/member/result types, strategy parser, family validation using the catalog taxonomy, bounded synthesis renderer, and bundled read-only neutral and persona panel agents. | Unit tests cover `@role` resolution, malformed role shapes, duplicate or unknown independent families, prompt size bounds, failure labels, and untrusted-text escaping.                                                                           |
