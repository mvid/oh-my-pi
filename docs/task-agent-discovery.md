...
- `restrictTools: true` is a hard allowlist: the subagent sees only its listed built-in tools plus `yield`; MCP, extensions, and custom tools are disabled. An explicit `tools` list is required — `restrictTools: true` without one is rejected and the agent is skipped.
